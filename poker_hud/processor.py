...
def process_session_file(
    ...
    Args:
        ...
        stat_calculators: Dictionary mapping Stat -> calculator function
                         (default: DEFAULT_STAT_CALCULATORS)
...
def aggregate_all_sessions(
    session_stats_list: list[Dict[str, Dict[Stat, Dict[str, Tuple[float, int]]]]]
) -> Dict[str, Dict[Stat, Tuple[float, int]]]:
    """
    Aggregate statistics across multiple sessions (position-collapsed).

    Combines session-level position-bucketed stats into overall stats for each player.
    Uses the "ALL" position from each session.

    Args:
        session_stats_list: List of session stat dictionaries (position-bucketed format)

    Returns:
        Dictionary mapping player -> (Stat -> (total_num, total_denom))
        Position-collapsed format using "ALL" position data.

    Example:
        >>> session1 = {"alice": {Stat.VPIP: {"ALL": (10, 20)}}}
        >>> session2 = {"alice": {Stat.VPIP: {"ALL": (5, 10)}}}
    ...
    for session_stats in session_stats_list:
        for player, stats in session_stats.items():
            ...
            for stat, positions in stats.items():
                # Use "ALL" position for overall aggregation
                if "ALL" in positions:
